...
import { flushBurn } from './core/fxu.js';
import { TERRAIN } from './world/terrain.js';
import { thunder, setLoops } from './fx/audio.js';
import { burnPlayer } from './game/player.js';
...
const STEPS = [
  ...
  ['Физика: статический мир', () => { const g = heightGrid(); buildStaticWorld(g.H, g.HN, g.HS, g.R); buildPlayerProxy(); }],
...
function finish() {
  ...
  window.MAP_API = {
    ...
    stats: () => ({
      ...
      mines: MINES.list.length, paths: PATHS.length, ...physStats(), ...fireStats(), ...weatherStats(), ...glassStats(), ...destructionStats(), trenches: TRENCHES.length, staticDraws: draws, ...lampStats(), quality: QNAME
    })
  };
...
function frame(dt, render = true) {
  ...
  updateFootprints(PL);
  flushBurn(FRAME.t);
  burnPlayer(dt, heatAt(PL.pos.x, PL.pos.z));
  ...
  updatePost(SKY, Math.min(1, BLAST.shake * 0.8 + (PL.dead > 0 ? 0.6 : 0) + PL.burn * 0.3), WEATHER.flash);
  if (!render) return;
  composer.render();
  updateHud(dt);
}
